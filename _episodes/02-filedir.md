...
Let's say we want to move to the `data` directory we saw above. We can //deleted extra space after period
...
But we get an error! Why is this? //deleted extra space after exclamation mark
...
the filesystem (including from inside `data`). To find the absolute path //deleted extra space after period
...
> then type, the full path.  This shortcut is a *very* efficient way of moving back 
